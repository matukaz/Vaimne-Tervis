			...
			if checked then
				...
				--	
					...
					if(itemsSelected < 6) then
  					...
  					print(itemsSelected .." TOGGLEBUTTON UI CALL")
					default.isVisible = false
					over.isVisible = false
...
function newButton( params )
	...
	if params.text then
		...
		button.text = params.text -- Added so i could check what text was added to button
	...
	function button:isChecked() -- return current state
		...
		-- Public methods
	function button:itemsSelected( number) -- return current state
		itemsSelected = number
	end

	function button:itemsSelectedNumber( ) -- return current state
